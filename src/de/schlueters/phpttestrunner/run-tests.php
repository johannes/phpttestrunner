...
function show_start($start_time)
{
	...
	if ($html_output)
	{
		fwrite($html_file, "<div><h2>Time Start: " . @date('Y-m-d H:i:s', $start_time) . "</h2>\n");
...
//		fwrite($html_file, "<tr><td colspan='3'>---&gt; $tests ($tested [$tested_file]) begin</td></tr>\n");
		fwrite($html_file, "<tr><td colspan='3'>---&gt; ".htmlspecialchars($tests)." (".htmlspecialchars($tested)." [$tested_file]) begin</td></tr>\n");
	}
...
//		fwrite($html_file, "<tr><td colspan='3'>---&gt; $tests ($tested [$tested_file]) done</td></tr>\n");
		fwrite($html_file, "<tr><td colspan='3'>---&gt; ".htmlspecialchars($tests)." (".htmlspecialchars($tested)." [$tested_file]) end</td></tr>\n");
	}
...
function show_result($result, $tested, $tested_file, $extra = '', $temp_filenames = null)
{
	...
	if ($html_output) {
		...
		if (isset($temp_filenames['file']) && file_exists($temp_filenames['file'])) {
			...
			$tested = "<a href='$url'>".htmlspecialchars($tested)."</a>";
		}
		...
		if (isset($temp_filenames['skip']) && file_exists($temp_filenames['skip'])) {
			...
			$extra = "<a href='$url'>".htmlspecialchars($extra)."</a>";
		...
		fwrite($html_file, 
			...
			"<td>".htmlspecialchars($tested)."</td>" .
			"<td>".htmlspecialchars($extra)."</td>" .
			"<td>".htmlspecialchars($diff)."</td>" . 
			"<td>".htmlspecialchars($mem)."</td>" .
			"<td>".htmlspecialchars($tested_file)."</td>" .
			"</tr>\n");
	}
}
